...
Приблизительный объем картинок, содержащийся в пинтересте: 240 *1 000 000 000 * 40кб /1024 /1024 /1024 = 8940.69тб. 
Согласно характеристикам, сейчас на рынке можно найти HDD с объемом до 20Tb. При факторе репликации 2 в ceph 
потребуется 17880тб.
На такой объем данных потребуется около 360 000 таких дисков.
...
Объем данных в Postgres: 
Для пользователя: 16 байт uuid, 200 байт user_info, following_list, follower_list - массив uuid
Для 500 млн пользователей: (5*1000000*(1501*16+200))/1024/1024/1024=112.76Гб

Для досок: 16 байт uuid * 2, 600 байт описания и меты, лист ссылок на посты
Для 5 миллиардов досок:(5*1000000000*(632+200*16))/1024/1024/1024/1024=17.42Тб

Для постов: 3 * 16 байт uuid, 600 байт описания и меты, 100 байт source_url + 2 uuid-а комментариев.
Для 250 миллиардов постов: (250*1000000000*(748+16*2))/1024/1024/1024/1024=177.35Тб 
В целом необходимо ~180Тб объема.

Для Apache Kafka: сообщение содержит user_id, search_request, opened_pin_id - итого максимум 1кб сообщения на 
событие. При 8К запросов в секунду необходимо 4Tb памяти, ключая репликацию. Фактор репликации 3.
Конфигурация Кафки согласно [калькулятору](https://eventsizer.io): 4 брокера, 5 zookepers, 2 connect worker-а.


*Таблица 1. Конфигурация сервера для Nginx*
CPU(cores)    | RAM (Gb)      |SSD (Gb)| Network (Gb/s)|Количество|
------------- | ------------- |--------|---------------|----------|
16            | 16            |128     |2,5            |4+4       |

*Таблица 2. Конфигурация сервера для хранения данных (Ceph)*
CPU(cores)    | RAM (Gb)      |SSD (Tb) | Network (Gb/s)|Количество|
------------- | ------------- |---------|---------------|----------|
64            | 128           |20 Tb SAS|2,5            |1000      |

*Таблица 3. Конфигурация сервера для хранения данных (Postgres)*

CPU(cores)    | RAM (Gb)      |SSD (Tb)| Network (Gb/s)|Количество|
------------- | ------------- |--------|---------------|----------|
64            | 128            |20Tb     |2,5           |20     |

*Таблица 4. Конфигурация сервера для хранения сессий (Redis)*

CPU(cores)    | RAM (Gb)      |SSD (Gb)| Network (Gb/s)|Количество|
------------- | ------------- |--------|---------------|----------|
16            | 128            |128     |2,5            |4+4       |

*Таблица 5. Конфигурация сервера для работы приложений (Go)*
CPU(cores)    | RAM (Gb)      |SSD (Gb)| Network (Gb/s)|Количество|
------------- | ------------- |--------|---------------|----------|
64            | 96            |128     |2,5            |4+4       |

*Таблица 5. Конфигурация сервера для работы рекомендаций (Kafka)*
CPU(cores)    | RAM (Gb)      |SSD (tb)| Network (Gb/s)|Количество|
------------- | ------------- |--------|---------------|----------|
24            | 128            |10       |10             |1       |
